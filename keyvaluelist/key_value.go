...
// ErrInvalidFormat is returend when FromHadoopPartition fails to parse the input
...
// ToHadoopPartition encodes the KeyValue pair into a hadoop style partion string with the key and value QueryEncoded
func (kv KeyValue) ToHadoopPartition() string {
	return url.QueryEscape(kv.Key) + "=" + url.QueryEscape(kv.Value)
}

// FromHadoopPartition encodes the KeyValue pair into a hadoop style partion string with the key and value QueryEncoded
